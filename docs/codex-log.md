...
|---|---|---|---|---|
| 2026-08-22 | 프로시저럴 사운드 신시사이저 (`src/audio/soundSynth.ts`) | 지시서 `docs/codex-tasks/03-sound-synth.md` — SoundSynth 클래스 인터페이스·사운드 8종 사양(험·심장·발소리·글리치·비프 2종·노크·속삭임) 고정, 오디오 파일 금지·전부 Web Audio 합성 | 260줄, 계약 준수 (외부 import 0, 파일 0개 — 빌드 용량 증가 0) | 사람이 인터페이스·각 사운드의 합성 레시피(주파수·필터·엔벨로프)를 지시서에 사전 확정. 게임 연결(씬별 훅, 발소리 트래커)은 별도 수행 |
